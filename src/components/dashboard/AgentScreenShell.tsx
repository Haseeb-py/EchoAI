...
import { AgentHeader, AgentSidebar } from "@/components/dashboard/AgentNavigation";

type AgentKey = "dashboard" | "campaigns" | "live" | "history" | "leads";

export function AgentScreenShell({
  activeKey,
  contextLabel,
  eyebrow,
  title,
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#080A13] text-white">
      <div className="flex min-h-screen bg-[radial-gradient(circle_at_24%_12%,rgba(118,107,255,0.14),transparent_34%),radial-gradient(circle_at_88%_20%,rgba(143,221,224,0.1),transparent_30%),#0D0F1A]">
        <AgentSidebar activeKey={activeKey} />
        <SupervisorLiveCallEngine />
        <main className="flex-1">
          <AgentHeader contextLabel={contextLabel} activeKey={activeKey} showContextLabel={showContextLabel} />
