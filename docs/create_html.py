    ...
    "children": [
        {
            ...
            "children": [
                ...
                {
                    "id": "AdminOverview", "name": "AdminOverview.tsx", "type": "file",
                    "role": "Dashboard View", "deps": "Lucide React",
                    "desc": "The landing page for Admins, displaying a quick snapshot of system health.",
                    "workflow": [
                        {"title": "Page Load", "desc": "Displays immediate top-line metrics: active users, total scripts, campaigns running."}
                    ],
                    "logic": "Currently uses local mock data to populate metric cards. Intended to fetch an aggregation endpoint in the future.",
                    "tech": [
                        {"name": "CSS Grid", "reason": "Used to layout the metric cards responsively."}
                    ]
                },
                {
                    "id": "AdminScreenShell", "name": "AdminScreenShell.tsx", "type": "file",
                    "role": "Wrapper Component", "deps": "React",
                    "desc": "Provides consistent padding, background styling, and generic components like `AdminStatCard` and `AdminMessage`.",
                    "workflow": [
                        {"title": "Wrapper usage", "desc": "Other admin pages wrap their content in this shell to inherit the dark-mode layout."}
                    ],
                    "logic": "Exports small, reusable presentation components to prevent code duplication across admin views.",
                    "tech": [
                        {"name": "React Composition", "reason": "Uses the `children` prop pattern to wrap generic content."}
                    ]
                },
                {
                ...
                {
                    "id": "KnowledgeBasePage", "name": "KnowledgeBasePage.tsx", "type": "file",
                    "role": "Feature View", "deps": "Mock State",
                    "desc": "Interface for Admins to upload PDFs, URLs, and text to train the AI's RAG (Retrieval-Augmented Generation) system.",
                    "workflow": [
                        {"title": "Upload", "desc": "Admin drags and drops a PDF or enters a website URL."},
                        {"title": "Processing", "desc": "UI shows a simulated progress bar indicating 'vectorizing' or 'chunking'."}
                    ],
                    "logic": "Purely frontend mock. In reality, this would send file blobs to a Python backend, which would use LangChain to chunk and store embeddings in a vector database.",
                    "tech": [
                        {"name": "File API", "reason": "Uses browser native APIs for handling drag-and-drop file objects."}
                    ]
                },
                {
                    "id": "LeadsCrmPage", "name": "LeadsCrmPage.tsx", "type": "file",
                    "role": "Data View", "deps": "Mock State",
                    "desc": "A miniature CRM for managing lead contact information and call history.",
                    "workflow": [
                        {"title": "Import", "desc": "Admins can bulk-import leads (mocked)."},
                        {"title": "Review", "desc": "View specific details, phone numbers, and dispositions of leads."}
                    ],
                    "logic": "Filters and sorts large arrays of objects in memory. No backend persistence yet.",
                    "tech": [
                        {"name": "Array Iteration", "reason": "Heavy use of map/filter to manage table states."}
                    ]
                },
                {
                    "id": "ReportsExportsPage", "name": "ReportsExportsPage.tsx", "type": "file",
                    "role": "Action View", "deps": "Mock Export",
                    "desc": "Allows Admins to generate and download CSV reports of call logs or agent performance.",
                    "workflow": [
                        {"title": "Selection", "desc": "Admin selects a date range and report type."},
                        {"title": "Download", "desc": "Simulates generating a file and prompting a browser download."}
                    ],
                    "logic": "Mocks the generation of CSV data. A real implementation would request a CSV stream from FastAPI.",
                    "tech": [
                        {"name": "Blob Generation", "reason": "Uses `URL.createObjectURL(new Blob(...))` to trigger browser downloads without a server in the prototype."}
                    ]
                },
                {
                    ...
                    "logic": "Two distinct forms and tables live within this file. The logic separates the concepts: Scripts dictate 'What the AI says' while Personas dictate 'How the AI sounds'.",
                    "tech": [
                        {"name": "SQLAlchemy Models", "reason": "The backend perfectly mirrors this UI with Script and Persona database models."}
                    ]
                },
                {
                    "id": "SecurityAccessPage", "name": "SecurityAccessPage.tsx", "type": "file",
                    "role": "Audit View", "deps": "Mock Data",
                    "desc": "Displays audit logs of who logged in, IP addresses, and security events.",
                    "workflow": [
                        {"title": "Review", "desc": "Admin scrolls through chronological security events."}
                    ],
                    "logic": "Static rendering of predefined mock audit logs.",
                    "tech": [
                        {"name": "Table Layouts", "reason": "Uses standard Tailwind table structures for log viewing."}
                    ]
                },
                {
        ...
        },
        {
            ...
            "children": [
                {
                    "id": "AlertsPage", "name": "AlertsPage.tsx", "type": "file",
                    "role": "Monitoring View", "deps": "Mock State",
                    "desc": "Dedicated page for viewing historical and active system alerts.",
                    "workflow": [
                        {"title": "Review Alerts", "desc": "Supervisors scroll through a feed of 'Critical', 'Warning', and 'Info' alerts regarding campaign status or system errors."}
                    ],
                    "logic": "Filters a static array of alert objects based on severity level.",
                    "tech": [
                        {"name": "Badges", "reason": "Uses colored Tailwind badges to quickly identify alert severity."}
                    ]
                },
                {
                ...
                {
                    "id": "CampaignMonitoringPage", "name": "CampaignMonitoringPage.tsx", "type": "file",
                    "role": "Analytics View", "deps": "Mock State",
                    "desc": "Deep dive into the performance of specific active campaigns.",
                    "workflow": [
                        {"title": "Select Campaign", "desc": "Supervisor picks a campaign from a dropdown to view its specific conversion rate, call volume, and drop rate."}
                    ],
                    "logic": "Updates visual KPI cards based on the selected campaign string.",
                    "tech": [
                        {"name": "Select Elements", "reason": "Standard HTML selects bound to React state for filtering."}
                    ]
                },
                {
                ...
                {
                    "id": "PerformancePage", "name": "PerformancePage.tsx", "type": "file",
                    "role": "Analytics View", "deps": "Mock State",
                    "desc": "Displays leaderboards and Quality Assurance (QA) scores for individual agents.",
                    "workflow": [
                        {"title": "Review", "desc": "Supervisors evaluate which agents have the highest close rates and lowest handle times."}
                    ],
                    "logic": "Sorts and ranks an array of mock agent performance objects.",
                    "tech": [
                        {"name": "Table Sorting", "reason": "Implements basic client-side sorting logic based on numeric metrics."}
                    ]
                },
                {
                ...
                {
                    "id": "SupervisorNavigation", "name": "SupervisorNavigation.tsx", "type": "file",
                    "role": "Layout Component", "deps": "Next.js Link",
                    "desc": "Sidebar and header for the Supervisor workspace.",
                    "workflow": [
                        {"title": "Navigation", "desc": "Routes the user between Cockpit, Escalations, and Dashboard."}
                    ],
                    "logic": "Manages active link styling based on current route.",
                    "tech": [
                        {"name": "Lucide React", "reason": "Icons specifically chosen to represent monitoring and team management."}
                    ]
                },
                {
                    "id": "SupervisorScreenShell", "name": "SupervisorScreenShell.tsx", "type": "file",
                    "role": "Wrapper Component", "deps": "React",
                    "desc": "Layout wrapper providing consistent UI spacing for supervisor screens.",
                    "workflow": [
                        {"title": "Wrapping", "desc": "Encloses child components."}
                    ],
                    "logic": "Purely presentational.",
                    "tech": [
                        {"name": "CSS Classes", "reason": "Standardizes margins, padding, and background gradients."}
                    ]
                },
                {
                    "id": "SupervisorSettingsPage", "name": "SupervisorSettingsPage.tsx", "type": "file",
                    "role": "Config View", "deps": "Local State",
                    "desc": "Allows supervisors to configure personal notification preferences.",
                    "workflow": [
                        {"title": "Toggle Settings", "desc": "Supervisors turn on/off browser notifications for escalations."}
                    ],
                    "logic": "Maintains boolean states for various preference toggles.",
                    "tech": [
                        {"name": "Toggle UI", "reason": "Uses custom Tailwind toggle switches."}
                    ]
                },
                {
                ...
                {
                    "id": "live-calls", "name": "live-calls.tsx", "type": "file",
                    "role": "Data Component", "deps": "Mock Arrays",
                    "desc": "A specific component dedicated to listing the currently active calls across the floor.",
                    "workflow": [
                        {"title": "Call List", "desc": "Provides a scrollable list of calls that can be clicked to open in the Cockpit."}
                    ],
                    "logic": "Filters out ended calls and only displays calls with status 'in-progress'.",
                    "tech": [
                        {"name": "Component Extraction", "reason": "Separated from the main dashboard to keep code modular and reusable."}
                    ]
                }
            ]
        },
        {
            ...
            "children": [
                {
                    "id": "AgentDashboard", "name": "AgentDashboard.tsx", "type": "file",
                    "role": "Wrapper Component", "deps": "React",
                    "desc": "A very thin wrapper or redirect component for the agent root path.",
                    "workflow": [
                        {"title": "Routing", "desc": "Often used just to establish the layout or redirect directly to workflows."}
                    ],
                    "logic": "Minimal logic, primarily acts as an entry point.",
                    "tech": [
                        {"name": "Next.js Layouts", "reason": "Part of the Next.js App Router paradigm."}
                    ]
                },
                {
                    "id": "AgentNavigation", "name": "AgentNavigation.tsx", "type": "file",
                    "role": "Layout Component", "deps": "Next.js Link",
                    "desc": "Header and minimal sidebar specifically tailored for the Agent, omitting admin/supervisor features.",
                    "workflow": [
                        {"title": "Navigation", "desc": "Provides links to Workflows and Settings."}
                    ],
                    "logic": "Manages user dropdowns and logout functionality.",
                    "tech": [
                        {"name": "Auth Hooks", "reason": "Interacts with `src/lib/auth.ts` to execute logouts."}
                    ]
                },
                {
                    "id": "AgentScreenShell", "name": "AgentScreenShell.tsx", "type": "file",
                    "role": "Wrapper Component", "deps": "React",
                    "desc": "Layout wrapper for Agent screens, focusing on maximum screen real-estate.",
                    "workflow": [
                        {"title": "Layout constraints", "desc": "Ensures the main workspace takes up the full viewport height to prevent scrolling during live calls."}
                    ],
                    "logic": "Presentational only.",
                    "tech": [
                        {"name": "Flexbox", "reason": "Uses `h-screen flex flex-col` to lock the UI layout tightly."}
                    ]
                },
                {
                ...
                },
                {
                    ...
                    "desc": "The agent's specific side navigation panel.",
                    "workflow": [
...
<head>
    ...
    <style>
        :root {
            ...
            --accent: #3b82f6; --accent-hover: #60a5fa; --border: #334155;
            --folder-color: #fbbf24; --file-color: #60a5fa; --success: #10b981;
        }
...
<body>
    ...
    <script>
        const codebaseData = __JSON_PLACEHOLDER__;
        ...
        function renderDetails(id) {
            ...
            if (node.type === 'folder') {
                panel.innerHTML = `
                    <div class="card">
                        <h2 class="detail-title">${icons.folder} ${escapeHTML(node.name)}</h2>
            ...
            let html = `
                <div class="card">
                    <h2 class="detail-title">${icons.file} ${escapeHTML(node.name)}</h2>
                    <div style="margin: 15px 0;">
                        <span class="badge role">Role: ${escapeHTML(node.role)}</span>
                        <span class="badge dep">Dependencies: ${escapeHTML(node.deps)}</span>
                    </div>
                    <p class="desc" style="font-size: 1.15rem; color: #fff;">${escapeHTML(node.desc)}</p>
                </div>
            `;
            ...
            if (node.logic) {
                ...
                <div class="card">
                    ...
                    <p class="desc">${escapeHTML(node.logic)}</p>
                </div>
                `;
            }
            ...
            if (node.tech && node.tech.length > 0) {
                ...
                <div class="card">
                    <div class="section-heading">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="12 2 2 7 12 12 22 7 12 2"></polygon><polyline points="2 17 12 22 22 17"></polyline><polyline points="2 12 17 22 12"></polyline></svg>
...
final_html = html_template.replace("__JSON_PLACEHOLDER__", json.dumps(codebaseData))

with open("e:/study/fyp/Updated/EchoAI/docs/EchoAI_Codebase_Explorer.html", "w", encoding="utf-8") as f:
    f.write(final_html)
